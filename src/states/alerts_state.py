...
alerts_state.py
---------------
THE ALERTS — one reminder at a time, big enough to read.
...
marks the one you are looking at as done.
    ...
    TOGGLE          SET DUE — the dial now moves the deadline in 15 min
                    steps instead of paging
    HOME button     back to Nexus

Reached from the DOCKET, which is the calm overview: the Docket tells you
whether anything needs you, this is where you deal with it. Two screens
because they answer different questions and one of them should be
...
The interrupt is separate again: states/alert_state.py (registered as
"dispatch") is what takes the screen when something arrives, and
backend/alerts.py decides when it is allowed to. Three names for three
different things — overview, list, interrupt — because collapsing any two
of them is what made the old single Docket unreadable.
"""
...
STAGE_COLOUR = {
    "POSTED": pal.CYAN,     "SCHEDULED": pal.CYAN,
    "BOARDING": pal.AMBER,  "TODAY": pal.ICE,
    "FINAL CALL": pal.MAGENTA, "DUE SOON": pal.AMBER,
    "DUE NOW": pal.MAGENTA, "OVERDUE": pal.BLOOD,
}


def due_text(due_ts, now=None):
    """A deadline as something you can act on, not a timestamp."""
    if not due_ts:
        return ""
    now = now or time.time()
    left = due_ts - now
    when = time.strftime("%H:%M", time.localtime(due_ts))
    day = time.strftime("%a", time.localtime(due_ts))
    same = time.strftime("%j", time.localtime(due_ts)) == time.strftime("%j",
                                                                       time.localtime(now))
    stamp = when if same else f"{day} {when}"
    if left < -60:
        return f"DUE {stamp}  ·  {int(-left // 60)} MIN LATE"
    if left < 60:
        return f"DUE {stamp}  ·  NOW"
    if left < 3600:
        return f"DUE {stamp}  ·  IN {int(left // 60)} MIN"
    if left < 86400:
        return f"DUE {stamp}  ·  IN {left / 3600:.0f} H"
    return f"DUE {stamp}"
...
class AlertsState(State):
    ...
    # ── data ───────────────────────────────────────────────────────────
    def items(self):
        """Open reminders, soonest deadline first, then oldest.

        A dated thing outranks an undated one: if you gave it a deadline
        you meant it to come first.
        """
        try:
    ...
    def enter(self):
        self.t = 0.0
        self.set_due_mode = bool(getattr(self.manager, "toggle_on", False))
        self.sel = 0
        self._laid_for = None
    ...
    def move_cursor(self, direction):
        ...
        With the toggle up the same dial nudges the deadline instead of
        paging — a deadline you can set on the device matters when the one
        you typed on your phone turns out to be wrong.
        """
        it = self.items()
        if not it:
            return True
        if self.set_due_mode:
            return self._nudge_due(direction)
        self.sel = (self.sel + (1 if direction > 0 else -1)) % len(it)
        self._laid_for = None
        return True

    def _nudge_due(self, direction):
        rec = self.current()
        if rec is None:
            return True
        step = 15 * 60 * (1 if direction > 0 else -1)
        base = rec.get("due_ts") or (time.time() + 15 * 60)
        new = max(time.time() - 3600, base + step)
        try:
            self.service.set_due(rec["id"], new)
        except Exception:                                   # noqa: BLE001
            return True
        self._flash(f"DUE {time.strftime('%H:%M', time.localtime(new))}")
        return True
    ...
    def on_toggle(self, on):
        self.set_due_mode = on
        self._flash("DIAL SETS DUE" if on else "DIAL PAGES")
    ...
    def draw(self, surface):
        ...
        stage = ("DELIVERED" if done
                 else stage_for(age, rec.get("due_ts"), now))
        ...
        due = due_text(rec.get("due_ts"), now)
        m = self.font_meta.render(due or self._age_text(age), True,
                                  accent if due else pal.INK_DIM)
        ...
        hint = ("DIAL SETS DUE  ·  PRESS DONE" if self.set_due_mode
                else "PRESS DONE  ·  DIAL PAGES  ·  RED SKIP")
    ...
    def _draw_pips(self, surface, items, y):
        ...
        for i, r in enumerate(items):
            ...
            _n = time.time()
            st = stage_for(max(0.0, _n - r.get("ts", _n)), r.get("due_ts"), _n)
    ...
    def _make_bg(self, size):
        ...
        pal.blit_glow(bg, self.font_title, "ALERTS", pal.CYAN, (s(14), s(9)))
        return bg
